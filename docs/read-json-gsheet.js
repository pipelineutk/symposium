 ...
 // ID of the Google Spreadsheet
 var spreadsheetID = "1gCubTVGtEpO02rSeuPHQiBezyeDhH2es9m7aZI0k_cI";
...
$.getJSON(url, function(data) {
  ...
  ** for (var i = entry.length - 1; i >= 0; i -= 1) {
  ...
  for (var i = entry.length - 1; i >= 0; i-=1) {
    ...
    oral_table += '<tr>';
    oral_table += '<td>' + entry[i]['gsx$firstname']['$t'] + '</td>';
    oral_table += '<td>' + entry[i]['gsx$lastname']['$t'] + '</td>';
    ...
    poster_table += '<tr>';
    poster_table += '<td>' + entry[i]['gsx$firstname']['$t'] + '</td>';
    poster_table += '<td>' + entry[i]['gsx$lastname']['$t'] + '</td>';
